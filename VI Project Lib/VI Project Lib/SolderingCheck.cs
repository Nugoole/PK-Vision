...
namespace VI_Project_Lib
{
    public partial class ImProcess
    {
        public void CircleDetect()
        {
            ...
            CircleSegment[] circles = Cv2.HoughCircles(processImg, HoughMethods.Gradient, 0.7, Math.Max(processImg.Cols,processImg.Rows) / 20, 10, 15, 9, 12);
            CircleSegment[] circles2 = Cv2.HoughCircles(processImg, HoughMethods.Gradient, 0.7, Math.Max(processImg.Cols, processImg.Rows) / 20, 10, 15, 6, 8);
            ...
            foreach (var circle in circles.OrderBy(x => x.Center.X).ToList())
            {
                ...
                Cv2.Circle(processImg, new Point((int)circle.Center.X, (int)circle.Center.Y), (int)circle.Radius, new Scalar(255,0,255), 1);
            ...
            {
                //Cv2.Circle(imgData, new OpenCvSharp.Point((int)circle.Center.X, (int)circle.Center.Y), 5, new Scalar(255,255,0));
                Cv2.Circle(processImg, new Point((int)circle.Center.X, (int)circle.Center.Y), (int)circle.Radius, new Scalar(0, 255, 255), 1);
                //Cv2.ImShow($"{Math.Min(processImg.Cols, processImg.Rows)}", processImg);
                //Cv2.WaitKey();
